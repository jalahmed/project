...
{
	if($needAuth && !is_logged()) $sqlWhere = 'WHERE published=1 and rep_id='.GetID();
	else $sqlWhere = 'WHERE rep_id='.GetID();
	$t = array();
	$t['total'] = 0;
...
elseif(isset($_GET['loadTasks']))
{
	...
	$t['total'] = 0;
	$t['list'] = array();
	$q = $db->dq("SELECT *, duedate IS NULL AS ddn FROM {$db->prefix}todolist $inner WHERE 1=1 $sqlWhere and rep_id=".GetID()." $sqlSort");
	while($r = $q->fetch_assoc($q))
	{
		$t['total']++;
...
elseif(isset($_GET['newTask']))
{
	...
	$db->ex("BEGIN");
	$db->dq("INSERT INTO {$db->prefix}todolist (uuid,list_id,title,d_created,d_edited,ow,prio,rep_id) VALUES (?,?,?,?,?,?,?,?)",
				array(generateUUID(), $listId, $title, time(), time(), $ow, $prio,GetID()) );
...
elseif(isset($_GET['addList']))
{
	...
	$db->dq("INSERT INTO {$db->prefix}lists (uuid,name,ow,d_created,d_edited,rep_id) VALUES (?,?,?,?,?,?)",
				array(generateUUID(), $name, $ow, time(), time(),GetID()) );
	$id = $db->last_insert_id();
	$t['total'] = 1;
