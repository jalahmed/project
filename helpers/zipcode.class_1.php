...
class zipcode_class {
  ...
  function get_zips_in_city($zip, $range, $sort=1,$include_base,$city,$ziploc) {
...
echo $city;
      ...
      $sql .= "lat BETWEEN '$min_lat' AND '$max_lat'
               AND lon BETWEEN '$min_lon' AND '$max_lon' and city !='$city' and zip_code IN $ziploc";
  echo $sql;
      $r = mysql_query($sql,$connection);
...
//echo "<pre>";
   ...
   function get_zip_city($city) {

      // This function pulls just the lattitude and longitude from the
      // database for a given zip code.
   $database = new connection();
            $connection = $database->getConnection();
      $sql = "SELECT zip_code from zip_code WHERE city='$city' limit 1";
      $r = mysql_query($sql,$connection);
      if (!$r) {
         $this->last_error = mysql_error();
         return false;
